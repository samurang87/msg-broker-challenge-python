...
The in-memory storage would be limiting in a real-world scenario, where a single instance of a message broker would need to be able to handle multiple connections, not to mention be able to scare horizontally with multiple instances. In this case, more appropriate storage solutions would be key-value stores like Redis or databases like PostgreSQL. I could also reduce the API layer to a thinner one on top of RabbitMQ or Kafka, which already satisfy by themselves most of the requirements of the broker.
Regarding the reviewer application, I would use a NoSQL database to store the logs, like Cassandra or Elasticsearch.
